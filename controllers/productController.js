...
export const getProducts = async (req, res) => {
  try {
  const { search, currency = 'USD', category, categories, isNew, isFeatured, minRating } = req.query;
    ...
    // Category filters
    if (categories) {
      // Accept comma-separated string or array from querystring
    ...
    // Top rated filter: minimum rating threshold
...
export const createProduct = async (req, res) => {
  try {
    ...
    const product = new Product({
      ...req.body,
      price: priceInUSD,
      // Accept optional video URL if provided
...
export const updateProduct = async (req, res) => {
  try {
  const { sizes, colors, video, ...rest } = req.body;
  const updateData = { ...rest };
  if (video !== undefined) updateData.video = video; // allow clearing by sending empty string
