...
export const getAnnouncements = async (req, res) => {
  try {
    ...
    if (req.query.platform) {
      ...
      if (['web', 'mobile', 'both'].includes(p)) {
        filter.$or = [{ platform: p }, { platform: 'both' }];
      }
    }
...
// Convenience dedicated mobile active route handler
export const getActiveMobileAnnouncements = async (req, res) => {
  try {
    const announcements = await Announcement.find({ isActive: true, $or: [ { platform: 'mobile' }, { platform: 'both' } ] })
      .sort('order')
      .select('-__v');
    res.json(announcements);
  } catch (error) {
