...
export const addReview = async (req, res) => {
  ...
  const { isValid, errors } = validateReviewData({ rating, comment, photos, fit, quality });
    if (!isValid) {
    ...
    const purchased = await hasUserPurchasedProduct(req.user._id, product._id);
    if (!purchased) {
...
export const updateReview = async (req, res) => {
  ...
  if (fit !== undefined) review.fit = fit;
...
export const getReviewEligibility = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to determine eligibility' });
  }
};